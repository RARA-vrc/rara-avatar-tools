            ...
            ConversionReport report)
        {
            ...
            try
            {
                ...
                foreach (var kv in byName)
                {
                    ...
                    {
                        // [C] アニメーションで動かしているシェイプは実行時にアニメが値を決めるため、この値の不定は
                        //     多くの場合無害である旨を明記する(ユーザーが不要に統合を諦めないよう案内を補強)。
                        if (isAnimated)
                        {
                            report.Warn($"同名ブレンドシェイプ '{name}' の値が統合対象間で異なります(アニメーション使用のため自動固定しません)。アニメーションで動かしているシェイプは実行時にアニメーションが値を決めるため、この不定は多くの場合無害です。見た目が意図と異なる場合のみ、該当メッシュを統合から除外してください。");
                        }
                        else
                        {
                            report.Warn($"同名ブレンドシェイプ '{name}' の値が統合対象間で異なります(ビセーム/まばたき/MMD標準モーフのため自動固定しません)。見た目が意図と違う場合は該当メッシュを統合から除外してください。");
                        }
                        continue;
                    }
